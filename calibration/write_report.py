...
def main(argv: list[str] | None = None) -> int:
    ...
    md += [
        f"No national ISIC × size counts could be pulled (§5). What can be said without them: the config implies an aggregate micro/small/medium split of "
    ...
    for s in ("retail_trade", "food_beverage", "construction", "professional_services", "total"):
        ...
        md.append(f"| {s} | {implied.get(s, float('nan')):.0f} | {fmt(row[0])} | **{fmt(row[1])}** | {fmt(row[2])} | {fmt(row[3])} |")
    md += [
        "",
    ...
    if emergent_before:
        b_lines = [ln for ln in emergent_before.splitlines() if ln.startswith("| ") or ln.startswith("**") or ln.startswith("Scorable") or ln.startswith("Realised")]
        md += b_lines[:40] + [""]
    ...
    def rvs_head(t: str | None, label: str) -> list[str]:
        ...
        keep = [ln for ln in t.splitlines() if ln.startswith("| Comparison-set AUC") or ln.startswith("**Verdict") or ln.startswith("| Default rate") or ln.startswith("| n businesses")]
        return [f"**{label}**", ""] + keep + [""]
